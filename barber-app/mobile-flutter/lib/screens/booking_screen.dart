...
class _BookingScreenState extends State<BookingScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.barbershopName),
      ),
      body: _isLoading
          ...
          : SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  Text('📅 Novo Agendamento', style: Theme.of(context).textTheme.displaySmall),
                  const SizedBox(height: 24),

                  // Barbeiro
                  Text('👨‍💼 Escolha o Barbeiro', style: Theme.of(context).textTheme.titleLarge),
                  const SizedBox(height: 12),
                  ..._barbers.map((barber) => Card(
                        margin: const EdgeInsets.only(bottom: 8),
                        color: _selectedBarber?.id == barber.id ? AppTheme.gold.withOpacity(0.2) : null,
                        child: ListTile(
                          leading: CircleAvatar(
                            backgroundColor: _selectedBarber?.id == barber.id ? AppTheme.gold : AppTheme.secondary,
                            child: Icon(
                              Icons.person,
                              color: _selectedBarber?.id == barber.id ? AppTheme.background : AppTheme.white,
                            ),
                          ),
                  ...
                  // Serviço
                  Text('✂️ Escolha o Serviço', style: Theme.of(context).textTheme.titleLarge),
                  const SizedBox(height: 12),
                  ..._services.map((service) => Card(
                        margin: const EdgeInsets.only(bottom: 8),
                        color: _selectedService?.id == service.id ? AppTheme.gold.withOpacity(0.2) : null,
                        child: ListTile(
                          title: Text(service.name),
                          subtitle: Text('${service.formattedDuration} • ${service.formattedPrice}'),
                  ...
                  // Data e Hora
                  Text('🕐 Data e Hora', style: Theme.of(context).textTheme.titleLarge),
                  const SizedBox(height: 12),
                  Row(
                    children: [
                      Expanded(
                      ...
                      Expanded(
                        child: Card(
                          child: ListTile(
                            leading: const Icon(Icons.access_time, color: AppTheme.gold),
